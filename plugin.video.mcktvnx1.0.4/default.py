...
base='http://mcktv.co.uk'
ADDON=xbmcaddon.Addon(id='plugin.video.mcktvnx')
...
def CATEGORIES():
    link = OPEN_URL('http://www.mcktv.co.uk/wizardnx.txt').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    addDir('Fresh Start','', 2, 'https://archive.org/download/freshstartbrm/freshstart.png', 'https://archive.org/download/freshstartbrm/freshstart.png','Perform Fresh Start')
...
def wizard(name,url,description):
    ...
    dp.create("MckTV Wizard","Downloading ",'', 'Please Wait')
